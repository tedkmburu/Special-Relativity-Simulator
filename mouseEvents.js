        ...
        if (screen.visibility == "visible" && !popupVisibile) // this will only run if none of the popups are visible 
        {
            ...
            if (screen.name == "Home")
            {
                ...
                objects.forEach(object => {
                    if (mousePosition.x > object.pos.x &&
                        mousePosition.y > object.pos.y &&
                        mousePosition.x < object.pos.x + object.size.x &&
                        mousePosition.y < object.pos.y + object.size.y
                        && object.type == "Train")
                    {
...
function mouseDragged() // this is an inbuilt p5 function, runs when the mouse is clicked down and moved in any direction
{
    ...
    screens.forEach(screen =>
    {
        ...
        {
            if (screen.name == "Home" && !buildMode)
            {
                let displacement = (mouseX - pmouseX) / 1;

                trackOffset += displacement;

                objects.forEach(object => {
                    object.pos.x += displacement;
                })
